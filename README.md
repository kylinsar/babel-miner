...
`--poll`：链上更新间隔，默认8秒，最低0.5秒。常规轮询每次只读 `state()`；HTTP 429 等瞬时错误不停止挖矿，GPU继续当前任务，并按至少15秒退避后再查。连续180秒都读不到链上状态才停止。找到解或签名前仍做完整 `state`+`targetAt` 校验；有运行截止时间时瞬时读错误会退避重试到截止，没有截止时间时最多3次。签名和交易广播不自动重试。网站默认 RPC `https://towerofbabel.fly.dev/rpc/mainnet` 是共享代理，容易429；菜单选项8可改官方 `https://rpc.mainnet.arc.io`。
